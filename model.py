...
class Linear_Feature_Fusion_Approximate2():
    ...
    def loss(self):
        ...
        for i, j in self.M:
            ...
            #pull += max(0,self.distance(x1_tilde,x2_tilde,xa,xb))
...
class Linear_Feature_Fusion_Approximate3():
    def __init__(self,X,M,V,gamma,margin,lamb,indim=None,regularization=0,seed=0):
        random.seed(0)
        torch.manual_seed(seed)
...
class Linear_Feature_Fusion_Approximate2_Batch():
    #main difference is that now we don't even include samples in loss that are outside our range
    #strict
    def __init__(self,X,M,V,L,gamma,margin,lamb,indim=None,regularization=0,seed=0):
        ...
        #self.coeffs = [[-3.5507169411544033, 5.007510260923221, -2.8600040042166497, 1.8188528918604154], [3.6282080345829653, -8.063566001389777, 6.0138261097265975, -0.4337306099760562]]
    ...
    def loss(self):
        ...
        batch_size = 64
        #random_index = random.randint(0,self.X.shape[0]//batch_size-1)
...
class Linear_Feature_Fusion_Batch():
    def __init__(self,X,M,V,L,gamma,margin,lamb,indim=None,regularization=0,seed=0):
        random.seed(0)
        torch.manual_seed(0)
        self.V = V
...
class Linear_Feature_Fusion_Batch_NoShuffle():
    def __init__(self,X,M,V,L,gamma,margin,lamb,indim=None,regularization=0,seed=0):
        random.seed(0)
        torch.manual_seed(0)
        self.V = V
    ...
    def loss(self):
        ...
        if self.index >= self.X.shape[0] - batch_size - 1:
            ...
            shuffle_order = [i for i in range(len(self.X.tolist())//4)]
            random.shuffle(shuffle_order)
            temp_X = []
            temp_L = []
            X_list = self.X.tolist()
            L_list = self.L.tolist()
            ...
            self.X = torch.tensor(temp_X)
            self.L = torch.tensor(temp_L)
...
class Linear_Feature_Fusion_Approximate2_Batch_NoShuffle():
    #main difference is that now we don't even include samples in loss that are outside our range
    #strict
    def __init__(self,X,M,V,L,gamma,margin,lamb,indim=None,regularization=0,seed=0):
        random.seed(0)
